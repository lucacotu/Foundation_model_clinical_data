...
def main(dataset_name, feature_event, feature_time, seed, tuning=False):    
    ...
    for preprocess_type in preprocess:
        ...
        c_index_train_scores_tabicl_tuned_cox = []
        c_index_test_scores_tabicl_tuned_cox = []
        c_index_train_scores_tabicl_tuned_rsf = []
        c_index_test_scores_tabicl_tuned_rsf = []
        ...
        for fold, (train_idx, test_idx) in enumerate(folds["folds"]): 
            ...
            if tuning:
                ...
                cox_params = load_params(ckpt_dir, "tabicl_cox_tuned")

                if cox_params is not None and ckpt_exists(ckpt_dir, "tabicl_cox_tuned"):
                    print(f"  [ckpt] Loading TabICL CoxPH Tuned from {ckpt_dir}")

                    cox = EmbeddingCoxPH(**cox_params)
                    load_net_state(cox.model.net, ckpt_dir / "tabicl_cox_tuned.pt", device=cox.model.device, model=cox.model)
                else:
                    ...
                    save_params(ckpt_dir, "tabicl_cox_tuned", best_params)
                    ...
                    callbacks = [tt.callbacks.EarlyStopping(patience=20,          
                                                            ...
                                                            file_path=str(ckpt_dir / "tabicl_cox_tuned_tmp.pt"), 
                                                            load_best=True,
                                                            )]
                    cox.fit(
                            train_embeddings,
                ...
                c_index_train_scores_tabicl_tuned_cox.append(c_train)
                c_index_test_scores_tabicl_tuned_cox.append(c_test)

                rsf_params = load_params(ckpt_dir, "tabicl_rsf_tuned")
                rsf_path   = ckpt_dir / "tabicl_rsf_tuned.pkl"

                if rsf_params is not None and rsf_path.exists():
                    print(f"  [ckpt] Loading TabICL RSF Tuned from {ckpt_dir}")
                    rsf_tuned = load(rsf_path)

                else:

                    study_name = "rsf_tuning_tabicl_{fold}_{type}_seed{seed}".format(fold=fold+1, type=preprocess_type, seed=seed)
                    ...
                    save_params(ckpt_dir, "tabicl_rsf_tuned", best_rsf_params)
                ...
                print(f"C-index train: {c_train:.4f}")
                print(f"C-index test:  {c_test:.4f}")
                c_index_train_scores_tabicl_tuned_rsf.append(c_train)
                c_index_test_scores_tabicl_tuned_rsf.append(c_test)
            
            
            # Simple CoxPH on the embeddings (without tuning)
            simple_path = ckpt_dir / "tabicl_cox_simple.pt"
            ...
            vanilla_path = ckpt_dir / "tabicl_cox_vanilla.pt"
            in_features  = train_embeddings.shape[1]
            num_nodes    = [32, 32]
            out_features = 1
            batch_norm   = True
        ...
        results.append( (preprocess_type, 
                        ...
                        (c_index_train_scores_tabicl_tuned_cox, c_index_test_scores_tabicl_tuned_cox),
                        (c_index_train_scores_tabicl_tuned_rsf, c_index_test_scores_tabicl_tuned_rsf),
...
def get_ckpt_dir(dataset_name: str, preprocess_type: str, seed: int, fold: int) -> Path:
    ...
    p = Path("checkpoints/tabicl") / dataset_name / preprocess_type / f"seed{seed}" / f"fold{fold}"
...
if __name__ == "__main__":
    ...
    for exp_idx, experiment in enumerate(res):
        ...
        all_train_tabicl_tuned_cox = []
        all_test_tabicl_tuned_cox = []
        all_train_tabicl_tuned_rsf = []
        all_test_tabicl_tuned_rsf = []
        ...
        for seed, result_list in experiment:
            for preprocess_type, (c_train_tabicl_vanilla, c_test_tabicl_vanilla), (c_train_tabicl_simple, c_test_tabicl_simple), (c_train_tabicl_tuned_cox, c_test_tabicl_tuned_cox), (c_train_tabicl_tuned_rsf, c_test_tabicl_tuned_rsf),(c_train_cox_vanilla, c_test_cox_vanilla),(c_train_cox_simple, c_test_cox_simple), (c_train_rsf, c_test_rsf) in result_list:
                ...
                if c_train_tabicl_tuned_cox:
                    print_stats("TabICL Train Tuned Cox", c_train_tabicl_tuned_cox)
                    print_stats("TabICL Test Tuned Cox",  c_test_tabicl_tuned_cox)
                if c_train_tabicl_tuned_rsf:
                    print_stats("TabICL Train Tuned RSF", c_train_tabicl_tuned_rsf)
                    print_stats("TabICL Test Tuned RSF",  c_test_tabicl_tuned_rsf)
                ...
                all_train_tabicl_tuned_cox.extend(c_train_tabicl_tuned_cox)
                all_test_tabicl_tuned_cox.extend(c_test_tabicl_tuned_cox)
                all_train_tabicl_tuned_rsf.extend(c_train_tabicl_tuned_rsf)
                all_test_tabicl_tuned_rsf.extend(c_test_tabicl_tuned_rsf)
        ...
        if len(all_train_tabicl_tuned_cox) > 0:
            print_stats("TabICL Train Tuned Cox", all_train_tabicl_tuned_cox)
            print_stats("TabICL Test Tuned Cox",  all_test_tabicl_tuned_cox)
        if len(all_train_tabicl_tuned_rsf) > 0:
            print_stats("TabICL Train Tuned RSF", all_train_tabicl_tuned_rsf)
            print_stats("TabICL Test Tuned RSF",  all_test_tabicl_tuned_rsf)
