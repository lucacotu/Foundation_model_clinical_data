...
def prepare_data(df_main):
    ...
    df_main['time'] = pd.qcut(df_main['FU'], q=4, labels=False, duplicates='drop')

    # 2. Crea la variabile di stratificazione combinata
    #    Es: evento=1, bin=2 → stratum "1_2"
    df_main['stratum'] = df_main['STATO_AL_FU'].astype(str) + '_' + df_main['time'].astype(str)
    ...
    #df_mortality = df_main.copy()
    cols_to_keep = binary_cols + ["FU", "STATO_AL_FU","SESSO0"]
    ...
    columns_to_drop = ["SAPEVA", "NF_IMA", "F_IMA", "NF_CBV", "F_CBV", "NF_HF", "F_HF","MORTE_CV"]
    #    "Data of death",
    #    "Fatal MI or Sudden death",  
    #    "UnKnown", 
    #    "Accident",
...
def prepare_cox_data_hurrah(df):
    """Prepares data for Cox mortality model."""
