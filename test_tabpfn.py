...
def main(dataset_name, feature_event, feature_time, seed, tuning=False):    
    ...
    for preprocess_type in preprocess:
        ...
        c_index_train_scores_tabpfn_tuned_cox = []
        c_index_test_scores_tabpfn_tuned_cox = []
        c_index_train_scores_tabpfn_tuned_rsf = []
        c_index_test_scores_tabpfn_tuned_rsf = []
        ...
        for fold, (train_idx, test_idx) in enumerate(folds["folds"]): 
            ...
            if tuning:
                ...
                cox_params = load_params(ckpt_dir, "tabpfn_cox_tuned")

                if cox_params is not None and ckpt_exists(ckpt_dir, "tabpfn_cox_tuned"):
                    print(f"  [ckpt] Loading TabPFN CoxPH Tuned from {ckpt_dir}")
                    cox = EmbeddingCoxPH(**cox_params)
                    load_net_state(cox.model.net, ckpt_dir / "tabpfn_cox_tuned.pt", device=cox.model.device, model=cox.model)
                    ...
                    save_params(ckpt_dir, "tabpfn_cox_tuned", best_params)
                    ...
                    callbacks = [tt.callbacks.EarlyStopping(patience=20,          
                                                            ...
                                                            file_path=str(ckpt_dir / "tabpfn_cox_tuned_tmp.pt"), 
                                                            load_best=True,
                                                            )]
                    cox.fit(
                        train_embeddings,
                ...
                c_index_train_scores_tabpfn_tuned_cox.append(c_train)
                c_index_test_scores_tabpfn_tuned_cox.append(c_test)

                rsf_params = load_params(ckpt_dir, "tabpfn_rsf_tuned")
                rsf_path   = ckpt_dir / "tabpfn_rsf_tuned.pkl"

                if rsf_params is not None and rsf_path.exists():
                    print(f"  [ckpt] Loading TabPFN RSF Tuned from {ckpt_dir}")
                    rsf_tuned = load(rsf_path)

                else:
                    study_name = "rsf_tuning_tabpfn_{fold}_{type}_seed{seed}".format(fold=fold+1, type=preprocess_type, seed=seed)
                    ...
                    save_params(ckpt_dir, "tabpfn_rsf_tuned", best_rsf_params)
                ...
                print(f"C-index train: {c_train:.4f}")
                print(f"C-index test:  {c_test:.4f}")
                c_index_train_scores_tabpfn_tuned_rsf.append(c_train)
                c_index_test_scores_tabpfn_tuned_rsf.append(c_test)
                
            # Simple CoxPH on the embeddings (without tuning)
            simple_path = ckpt_dir / "tabpfn_cox_simple.pt"
            ...
            vanilla_path = ckpt_dir / "tabpfn_cox_vanilla.pt"
            in_features  = train_embeddings.shape[1]
            num_nodes    = [32, 32]
            out_features = 1
            batch_norm   = True
            ...
            if vanilla_path.exists():
                ...
                #model_vanilla = load(vanilla_path)
            ...
            if preprocess_type != "NaN":
                ...
                if cox_vanilla_base_path.exists():
                    ...
                    #model_vanilla = load(cox_vanilla_base_path)
                    ...
                    #dump(model_vanilla, cox_vanilla_base_path)

                surv_test = model_vanilla.predict_surv_df(X_test.values.astype(np.float32))
                surv_train = model_vanilla.predict_surv_df(X_train.values.astype(np.float32))
                ...
                if cox_simple_base_path.exists():
                    ...
                    #model_simple = load(cox_simple_base_path)
        ...
        results.append( (preprocess_type, 
                        ...
                        (c_index_train_scores_tabpfn_tuned_cox, c_index_test_scores_tabpfn_tuned_cox),
                        (c_index_train_scores_tabpfn_tuned_rsf, c_index_test_scores_tabpfn_tuned_rsf),
...
def get_ckpt_dir(dataset_name: str, preprocess_type: str, seed: int, fold: int) -> Path:
    ...
    p = Path("checkpoints/tabpfn") / dataset_name / preprocess_type / f"seed{seed}" / f"fold{fold}"
...
if __name__ == "__main__":
    ...
    for exp_idx, experiment in enumerate(res):
        ...
        all_train_tabpfn_tuned_cox = []
        all_test_tabpfn_tuned_cox  = []
        all_train_tabpfn_tuned_rsf = []
        all_test_tabpfn_tuned_rsf  = []
        all_train_cox_vanilla    = []
        all_test_cox_vanilla     = []
        all_train_cox_simple    = []
        all_test_cox_simple     = []
        all_train_rsf    = []
        all_test_rsf     = []

        for seed, result_list in experiment:
            for preprocess_type, (c_train_tabpfn_vanilla, c_test_tabpfn_vanilla),(c_train_tabpfn_simple, c_test_tabpfn_simple),(c_train_tabpfn_tuned_cox, c_test_tabpfn_tuned_cox),(c_train_tabpfn_tuned_rsf, c_test_tabpfn_tuned_rsf),(c_train_cox_vanilla, c_test_cox_vanilla),(c_train_cox_simple, c_test_cox_simple), (c_train_rsf, c_test_rsf) in result_list:
                ...
                if c_train_tabpfn_tuned_cox:
                    print_stats("TabPFN Train Tuned Cox", c_train_tabpfn_tuned_cox)
                    print_stats("TabPFN Test Tuned Cox",  c_test_tabpfn_tuned_cox)
                if c_train_tabpfn_tuned_rsf:
                    print_stats("TabPFN Train Tuned RSF", c_train_tabpfn_tuned_rsf)
                    print_stats("TabPFN Test Tuned RSF",  c_test_tabpfn_tuned_rsf)
                if c_train_cox_vanilla:
                    print_stats("Cox Train", c_train_cox_vanilla)
                    print_stats("Cox Test",  c_test_cox_vanilla)
                if c_train_cox_simple:
                    print_stats("Cox Train", c_train_cox_simple)
                    print_stats("Cox Test",  c_test_cox_simple)
                if c_train_rsf:
                ...
                all_train_tabpfn_tuned_cox.extend(c_train_tabpfn_tuned_cox)
                all_test_tabpfn_tuned_cox.extend(c_test_tabpfn_tuned_cox)
                all_train_tabpfn_tuned_rsf.extend(c_train_tabpfn_tuned_rsf)
                all_test_tabpfn_tuned_rsf.extend(c_test_tabpfn_tuned_rsf)
                all_train_cox_vanilla.extend(c_train_cox_vanilla)
                all_test_cox_vanilla.extend(c_test_cox_vanilla)
                all_train_cox_simple.extend(c_train_cox_simple)
                all_test_cox_simple.extend(c_test_cox_simple)
        ...
        if len(all_train_tabpfn_tuned_cox) > 0:
            print_stats("TabPFN Train Tuned Cox", all_train_tabpfn_tuned_cox)
            print_stats("TabPFN Test Tuned Cox",  all_test_tabpfn_tuned_cox)
        if len(all_train_tabpfn_tuned_rsf) > 0:
            print_stats("TabPFN Train Tuned RSF", all_train_tabpfn_tuned_rsf)
            print_stats("TabPFN Test Tuned RSF",  all_test_tabpfn_tuned_rsf)
        print_stats("Cox Vanilla Train",    all_train_cox_vanilla)
        print_stats("Cox Vanilla Test",     all_test_cox_vanilla)
        print_stats("Cox Simple Train ",    all_train_cox_simple)
        print_stats("Cox Simple Test",     all_test_cox_simple)
